from fastapi import APIRouter
from app.api.v1.endpoints import (
    auth, academic_calendar, courses, enrollment, grading,
    research, notifications, departments, admit_card, admission,
    credit_details, orientation, ppw, students, synopsis, external_examiner, thesis,
)

api_router = APIRouter(prefix="/api/v1")
api_router.include_router(auth.router)
api_router.include_router(departments.router)
api_router.include_router(departments.admin_router)
api_router.include_router(academic_calendar.router)
api_router.include_router(courses.router)
api_router.include_router(enrollment.router)
api_router.include_router(grading.router)
api_router.include_router(research.router)
api_router.include_router(notifications.router)
api_router.include_router(admit_card.router)
api_router.include_router(admission.router)
api_router.include_router(credit_details.router)
api_router.include_router(orientation.router)
api_router.include_router(ppw.router)
api_router.include_router(students.router)
api_router.include_router(synopsis.router)
api_router.include_router(external_examiner.router)
api_router.include_router(thesis.router)
