...
import { ChevronLeft, ChevronRight, GraduationCap, LogOut, KeyRound, Repeat } from "lucide-react";
import { NAV, DASHBOARD_ROUTE } from "@/lib/navigation";
...
export function AMSSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  async function switchRole(assignmentId: string) {
    ...
    try {
      ...
      qc.clear();
      // Every successful switch lands on the Dashboard (product requirement):
      // the page being viewed may not be available to the new active
      // assignment. Only reached after the backend accepted the switch.
