...
export default function CoursesPage() {
  ...
  const [tab, setTab] = useState<"courses" | "offerings" | "availability">("courses");
  ...
  // Course Availability (course-availability task) — HOD/Admin only. Manages
  // ams_course_availability rows for the caller's OWN department, never
  // changing Course.department_id (ownership).
  ...
  const { data: availableToMe = [], isLoading: availabilityLoading } = useQuery<AvailableToMeRow[]>({
    queryKey: ["ams-courses-available-to-me"],
    queryFn: async () => (await api.get("/courses/available-to-me")).data,
    enabled: tab === "availability",
  });

  const { data: availabilitySearchResults = [] } = useQuery<SearchCourseResult[]>({
    queryKey: ["ams-courses-search", availabilitySearch],
    queryFn: async () => (await api.get("/courses/search", { params: availabilitySearch ? { q: availabilitySearch } : {} })).data,
    enabled: showAddExisting,
  });

  const addAvailability = useMutation({
    mutationFn: (courseId: string) => api.post(`/courses/${courseId}/availability`, {}),
    onSuccess: () => {
      toast.success("Course made available to your department.");
      qc.invalidateQueries({ queryKey: ["ams-courses-available-to-me"] });
      setShowAddExisting(false); setAvailabilitySearch("");
    },
  ...
  const removeAvailability = useMutation({
    mutationFn: ({ courseId, departmentId }: { courseId: string; departmentId: string }) =>
      api.delete(`/courses/${courseId}/availability/${departmentId}`),
    onSuccess: () => { toast.success("Availability removed."); qc.invalidateQueries({ queryKey: ["ams-courses-available-to-me"] }); },
    onError: (e: unknown) => toast.error((e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? "Failed to remove."),
  });
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-700 text-base mt-1">
            {tab === "courses" ? "Course catalog and credit structures" : tab === "offerings" ? "Offer courses for masters and PhD degree programme" : "Other departments' courses made available to your department's students"}
          </p>
        </div>
        ...
        {canManage && tab === "availability" && (
          <button onClick={() => setShowAddExisting(true)}
            className="flex items-center gap-2 px-4 py-2.5 bg-[#0D6E6E] text-white rounded-xl font-semibold text-base hover:bg-[#178F8F]">
            <Plus size={16} /> Add Existing Course
          </button>
        )}
      </div>
      ...
      <div className="flex gap-2 mb-5">
        {(canManage ? (["courses","offerings","availability"] as const) : (["courses","offerings"] as const)).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "courses" ? "Course Management" : t === "offerings" ? "Offer Course" : "Available to Department"}
          </button>
        ))}
      </div>
      ...
      {/* Add Existing (other-department) Course modal */}
      {showAddExisting && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-bold mb-1">Add Existing Course</h3>
            <p className="text-sm text-gray-600 mb-4">
              Search the full course catalogue and make a course available to your department. The course keeps its actual owning department — this does not transfer ownership.
            </p>
            <div className="relative mb-2">
              <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input autoFocus value={availabilitySearch} onChange={(e) => setAvailabilitySearch(e.target.value)} placeholder="Search course code or title…"
            ...
            <button onClick={() => { setShowAddExisting(false); setAvailabilitySearch(""); }} className="mt-4 w-full py-2.5 border border-gray-200 rounded-xl text-base font-medium hover:bg-gray-50">Close</button>
          </div>
        </div>
      )}
